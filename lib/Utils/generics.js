...
// alias biar jelas ini yg dipake device Android (format 3EB0 + hash)
const generateMessageIDAndroid = (userId) => generateMessageIDV2(userId);
exports.generateMessageIDAndroid = generateMessageIDAndroid;
// speculative - format id gaya iOS/Safari: hex uppercase polos tanpa prefix "3EB0"
// (dipake modded-baileys lain buat nyamain "keliatan kirim dari iPhone/Safari")
const generateMessageIDIos = (userId) => {
    const data = Buffer.alloc(8 + 20 + 16);
    data.writeBigUInt64BE(BigInt(Math.floor(Date.now() / 1000)));
    if (userId) {
        const id = (0, WABinary_1.jidDecode)(userId);
    ...
    random.copy(data, 28);
    const hash = (0, crypto_1.createHash)('sha256').update(data).digest();
    return hash.toString('hex').toUpperCase().substring(0, 20);
};
exports.generateMessageIDIos = generateMessageIDIos;
